...
function buildWordColumns(block: PrimitiveTranslationBlock) {
  ...
  const rawWidths = wordLengths.map((length) => Math.max(720, Math.floor((availableWidth * length) / totalLength)))
...
export async function downloadPrimitiveTranslationDocx({
  ...
  for (const block of blocks) {
    ...
    const sourceRow = new TableRow({
      children: wordColumns.map((width, index) => createWordCell(block.sourceTokens[index] ?? '', width, true)),
    })
    const translationRow = new TableRow({
      children: wordColumns.map((width, index) => createWordCell(block.translatedTokens[index] ?? '', width, false)),
    })
