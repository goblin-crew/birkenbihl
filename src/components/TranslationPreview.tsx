...
export function TranslationPreview({ blocks }: TranslationPreviewProps) {
  if (blocks.length === 0) {
    return (
      <div className="empty-state">
        ...
        <p>Translate some sentences to see the source and translated words line up here.</p>
      </div>
    )
  }
  ...
  return (
    <div className="sentence-stack">
      {blocks.map((block) => {
        const columnCount = Math.max(block.sourceTokens.length, block.translatedTokens.length)
        ...
        return (
          <article className="sentence-card" key={block.id}>
            <div className="sentence-row sentence-row--source">
              ...
              <div
                  className="word-grid"
                  style={{ gridTemplateColumns: `repeat(${columnCount}, minmax(8ch, 1fr))` }}
              >
                {columns.map((columnIndex) => (
                  <div className="word-cell" key={`${block.id}-source-${columnIndex}`}>
            ...
            <div className="sentence-row sentence-row--translation">
              ...
              >
                {columns.map((columnIndex) => (
                  <div className="word-cell word-cell--translation" key={`${block.id}-translation-${columnIndex}`}>
